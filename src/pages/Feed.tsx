...
import { X, Heart, Loader2, PawPrint, MessageCircle, Star, Info, ChevronDown, DollarSign } from 'lucide-react';
import { motion, useMotionValue, useTransform, PanInfo, useAnimation, AnimatePresence } from 'motion/react';
...
const SponsorshipModal = ({ pet, onClose, onSuccess }: { pet: Pet, onClose: () => void, onSuccess: (amount: number) => void }) => {
    ...
    >
      ...
      >
        <div className="relative p-6 flex flex-col items-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2 text-center">Виртуальный опекун</h2>
          <p className="text-sm text-app-dim text-center mb-6">Спонсируйте {pet.name}, чтобы помочь приюту с расходами.</p>
          ...
          <AnimatePresence mode="wait">
            {success ? (
              ...
              >
                ...
                <div className="text-app-dim text-sm text-center">Ваш вклад добавлен.</div>
              </motion.div>
            ) : (
              <motion.div
                key="form"
              ...
              >
                ...
                >
                  {processing ? <Loader2 className="w-5 h-5 animate-spin" /> : `Пожертвовать ${amount} ₽`}
                </button>
              </motion.div>
            )}
          </AnimatePresence>
...
const PetDetailsModal = ({ pet, onClose, onSponsor }: { pet: Pet, onClose: () => void, onSponsor: () => void }) => {
  return (
    <motion.div
    ...
    >
      <div className="relative h-[45%] shrink-0 bg-app-card">
        <img 
          src={pet.image_urls?.[0] || 'https://images.unsplash.com/photo-1543466835-00a7907e9de1?q=80&w=1000'} 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-app-bg via-app-bg/60 to-transparent flex flex-col justify-end p-8 pb-4">
          <h2 className="text-4xl text-white font-extrabold mb-1">{pet.name}, {pet.age} лет</h2>
          {pet.matchPercentage && (
      ...
      <div className="flex-1 flex flex-col bg-app-bg">
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <span className={`px-4 py-2 rounded-xl text-xs font-bold ${pet.attributes?.requires_space ? 'bg-indigo-500/20 text-indigo-300' : 'bg-white/5 text-app-text'}`}>
              {pet.attributes?.requires_space ? 'Нужен просторный дом' : 'Подходит для квартиры'}
            </span>
            <span className={`px-4 py-2 rounded-xl text-xs font-bold ${pet.attributes?.cat_friendly ? 'bg-app-success/20 text-app-success' : 'bg-rose-500/20 text-rose-300'}`}>
          ...
          <div className="bg-white/5 rounded-2xl p-5 mb-8 border border-white/5">
            <div className="flex justify-between font-bold mb-3 text-sm">
              ...
              <span>{pet.current_donations || 0} / {pet.donation_goal || 1} ₽</span>
            </div>
            <div className="h-2 bg-white/10 rounded-full overflow-hidden mb-3">
              <motion.div 
                className="h-full bg-gradient-to-r from-app-accent to-indigo-400" 
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(((pet.current_donations || 0) / (pet.donation_goal || 1)) * 100, 100)}%` }}
                transition={{ duration: 1, type: "spring" }}
              />
            </div>
            <p className="text-xs text-app-dim">Вы можете стать виртуальным опекуном и помогать приюту с содержанием.</p>
          </div>
        </div>
...
const TinderCard = ({ 
  ...
  const matchColor = pet.matchPercentage && pet.matchPercentage >= 85 
    ...
    >
      ...
      >
        <div 
          ...
          style={{ 
            backgroundImage: `linear-gradient(to bottom, transparent 60%, rgba(15, 23, 42, 0.95) 100%), url('${pet.image_urls?.[0] || 'https://images.unsplash.com/photo-1543466835-00a7907e9de1?q=80&w=1000'}')` 
          }}
        ></div>
        
        {isTop && pet.matchPercentage !== undefined && (
          <div className={`absolute top-6 left-6 px-3 py-1.5 rounded-full text-sm font-bold backdrop-blur-md border flex items-center gap-1.5 ${matchColor}`}>
        ...
        <div className="absolute bottom-0 left-0 w-full pb-8 pt-24 px-8 hidden md:flex flex-col z-10 pointer-events-none bg-gradient-to-t from-[#0F172A] to-transparent">
          <div className="flex justify-between items-end w-full">
            <div className="flex-1 pr-4">
              <h2 className="text-4xl font-extrabold mb-1 line-clamp-1">{pet.name}, {pet.age}</h2>
              <p className="text-base text-app-dim mb-4 line-clamp-1">{pet.description}</p>
            </div>
            {isTop && (
              <button 
                onPointerDown={(e) => { e.stopPropagation(); onInfo(pet); }}
          ...
          {pet.donation_goal > 0 && (
            <div className="bg-white/5 rounded-2xl p-4 backdrop-blur-sm pointer-events-auto border border-white/5">
              <div className="flex justify-between text-xs mb-2">
                ...
                <span>{pet.current_donations || 0} / {pet.donation_goal} ₽</span>
              </div>
              <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-app-accent to-indigo-400" style={{ width: `${(pet.current_donations / pet.donation_goal) * 100}%` }}></div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 block md:hidden z-10 pointer-events-none bg-gradient-to-t from-[#0F172A] to-transparent pt-12">
           <div className="flex justify-between items-end">
              <div className="flex-1 pr-2">
                <h2 className="text-3xl font-extrabold mb-1 line-clamp-1 text-white">{pet.name}, {pet.age}</h2>
  ...
  if (effectiveRole === 'SHELTER') {
    return (
      <div className="w-full max-w-[420px] h-[580px] flex items-center justify-center flex-col text-center p-8 bg-app-card rounded-[32px] border border-white/10 shadow-2xl">
        ...
        <p className="text-app-dim text-sm">Свайпы доступны только для усыновителей. Отслеживайте своих питомцев в разделе Дашборд.</p>
      </div>
    );
  }
        ...
        {loading ? (
          ...
          pets.map((pet, index) => (
           ...
           <div className="absolute inset-0 bg-app-card rounded-[32px] border border-white/10 flex flex-col items-center justify-center shadow-2xl p-8 text-center text-app-dim z-10">
             ...
             <button onClick={fetchFeed} className="mt-6 px-4 py-2 bg-white/5 rounded-xl text-sm font-bold text-app-text hover:bg-white/10 transition-colors">Обновить</button>
           </div>
        )}
