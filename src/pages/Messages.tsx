...
export default function Messages() {
    ...
    try {
      const { data: matchesData, error } = await supabase
        ...
        .select(
          `
          *,
          user:profiles!matches_user_id_fkey(name),
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      setMatches((prev) =>
        prev.map((m) => (m.id === activeMatch.id ? updatedMatch : m)),
      );
  ...
  return (
    <div className="w-full h-full flex bg-app-card rounded-[32px] border border-white/10 shadow-2xl overflow-hidden max-w-5xl mx-auto">
      <div
        className={`w-full md:w-[320px] shrink-0 border-r border-white/5 flex flex-col ${activeMatch ? "hidden md:flex" : "flex"}`}
      >
            ...
            matches.map((match) => (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-app-bg overflow-hidden border border-white/10 shrink-0">
                  <img
                    src={match.pet?.image_urls?.[0]}
                    alt="Pet"
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="font-bold truncate text-sm text-white flex items-center gap-2">
                    {effectiveRole === "SHELTER"
                      ? match.user?.name || "Усыновитель"
                      : match.shelter?.name || "Приют"}
                    {match.is_sponsor && (
                      <span className="bg-app-accent/20 text-app-accent text-[10px] px-2 py-0.5 rounded-full border border-app-accent/30 uppercase tracking-wider shrink-0">
                        Спонсор
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-app-dim truncate">
                    Питомец: {match.pet?.name || "Неизвестно"}
                  </div>
                </div>
                {match.shelter_rating && (
                  <Star
                    size={14}
      ...
      <div
        className={`flex-1 flex flex-col bg-app-bg/50 relative ${!activeMatch ? "hidden md:flex" : "flex"}`}
      >
        {!activeMatch ? (
          ...
          <>
            <div className="p-4 border-b border-white/5 bg-app-card/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 rounded-full overflow-hidden border border-white/10">
                  <img
                    src={activeMatch.pet?.image_urls?.[0]}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div>
                  <div className="font-bold text-white flex items-center gap-2">
                    {effectiveRole === "SHELTER"
                      ? activeMatch.user?.name || "Усыновитель"
                      : activeMatch.shelter?.name || "Приют"}
                    {activeMatch.is_sponsor && (
                      <span className="bg-app-accent/20 text-app-accent text-[10px] px-2 py-0.5 rounded-full border border-app-accent/30 uppercase tracking-wider">
                        Спонсор
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-app-dim">
                    Питомец: {activeMatch.pet?.name}
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    <span className="text-xs text-app-dim mb-1 px-1">
                      {senderName}
                    </span>
                    <div
                      className={`max-w-[80%] p-4 rounded-2xl ${
                        isMe
                          ? "bg-app-accent text-app-bg font-medium rounded-br-sm"
                          : "bg-white/5 text-app-text border border-white/10 rounded-bl-sm"
                      }`}
                    >
                      {msg.text}
                    </div>
                    <span className="text-[10px] text-app-dim mt-1 px-1">
            ...
            <form
              ...
              className="p-4 border-t border-white/5 bg-app-card/80"
            >
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="w-12 h-12 flex items-center justify-center bg-app-accent text-app-bg rounded-xl hover:scale-105 transition-transform active:scale-95"
                >
                  <Send size={20} />
                </button>
              </div>
